  ...
  module.exports = Snockets = (function() {
    ...
    Snockets.prototype.getCompiledChain = function(filePath, flags, callback) {
      ...
      return this.updateDirectives(filePath, flags, function(err, graphChanged) {
        ...
        _this.saveCacheFile(function(err) {
          return typeof callback === "function" ? callback(err, compiledChain, graphChanged) : void 0;
        });
    ...
    Snockets.prototype.getConcatenation = function(filePath, flags, callback) {
      ...
      return this.updateDirectives(filePath, flags, function(err, graphChanged) {
        ...
        _this.saveCacheFile(function(err) {
          return typeof callback === "function" ? callback(err, result, concatenationChanged) : void 0;
        });
        return result;
      });
    };
    ...
    Snockets.prototype.setCacheFile = function(filepath) {
      var caches, data;
      this.cacheFile = this.absPath(filePath);
      if (!fs.existsFileSync(this.cacheFile)) {
        return;
      }
      data = fs.readFileSync(this.cacheFile);
      try {
        caches = JSON.parse(data.toString('utf8'));
        this.cache = caches.cache;
        return this.concatCache = caches.concatCache;
      } catch (err) {
        return console.log(err);
      }
    };

    Snockets.prototype.saveCacheFile = function(callback) {
      if (this.cacheFile == null) {
        return callback(null);
      }
      return fs.writeFile(this.cacheFile, JSON.stringify({
        cache: this.cache,
        concatCache: this.concatCache
      }), function(err) {
        return callback(err);
      });
    };
